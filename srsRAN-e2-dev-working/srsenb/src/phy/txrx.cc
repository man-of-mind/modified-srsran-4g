...
void txrx::run_thread()
{
  ...
  while (running) {
    ...
    // uint iq_counter_modulus = 1000;
    // f = fopen("/mnt/tmp/agent_cmd.bin", "r");
    // if (f) {
    //   fseek(f, 1, SEEK_SET);
    //   size_t iqcm_s = fread((void *)&iq_counter_modulus, sizeof(uint), 1, f);

    //   if (iqcm_s != 1) {
    //     // srsran::console("I/Q counter modulus not read properly: %d\n", iqcm_s);
    //   } else {
    //     if (iq_counter_modulus > 3000)
    //       iq_counter_modulus = 3000;
    //     // srsran::console("I/Q counter modulus set to %d\n", iq_counter_modulus);
    //   }
    // }
    // fclose(f);


    iq_counter = (iq_counter + 1) % 2000;
    // std::cout<<iq_counter<<"checking iq_countyer"<<std::endl;
