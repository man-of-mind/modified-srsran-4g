...
zmq::context_t context_main(1); 
zmq::socket_t publisher_main(context_main, ZMQ_PUB) ; 
...
{
  publisher_main.bind("ipc:///tmp/socket_snr_cqi"); 
  started = false;
  phy_h   = phy;
  rlc_h   = rlc;
  rrc_h   = rrc;
...
void mac::ric_comm() {
    ...
    tti_ran_index += 1;
    tti_ran_index = tti_ran_index%2000;


    std::string send_message_to_er;

    // Initialize the message with the RAN index and RIC index
    send_message_to_er = std::to_string(tti_ran_index) + " ";

    // Log UE metrics to file
    std::ofstream logFile("ue_metrics.log", std::ios::out | std::ios::app);
    if (!logFile.is_open()) {
    ...
    for (auto& ue_pair : ue_db) {
        ...
        uint16_t rnti = ue_pair.first; // uint16_t rnti;
        uint8_t cqi = static_cast<uint8_t>(ue_metrics.dl_cqi);
        float snr_ue = snr_ul_ues[rnti];

        uint8_t rx_brate = static_cast<uint8_t>(ue_metrics.rx_brate);
        uint8_t rx_errors = static_cast<uint8_t>(ue_metrics.rx_errors);
        uint8_t rx_packets = static_cast<uint8_t>(ue_metrics.rx_pkts);
        uint8_t ue_buf = static_cast<uint8_t>(ue_metrics.ul_buffer);
        uint32_t tti_count = static_cast<uint32_t>(ue_metrics.nof_tti);

        uint8_t tx_brate = static_cast<uint8_t>(ue_metrics.tx_brate);
        uint8_t tx_errors = static_cast<uint8_t>(ue_metrics.tx_errors);
        uint8_t tx_packets = static_cast<uint8_t>(ue_metrics.tx_pkts);
        ...
        logFile << "RNTI: " << static_cast<int>(rnti)
                ...
                << ", UL buffer: " << static_cast<int>(ue_buf) << std::endl;
        
        
        send_message_to_er += std::to_string(rnti) + " " + std::to_string(cqi) + " " + std::to_string(snr_ue) + " ";
        }

        logFile.close();


    // Finalize and send the message using ZMQ publisher
    int m_size = send_message_to_er.size();
    zmq::message_t message(m_size);
    snprintf((char *)message.data(), m_size, "%s", send_message_to_er.c_str());
    publisher_main.send(message, zmq::send_flags::none);
