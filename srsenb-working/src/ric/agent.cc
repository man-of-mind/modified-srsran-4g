			   ...
			   int flags)
{
  ...
  printf("received e2-lite message: %.*s\n", pdu->N_bytes, pdu->msg);
  // agent_cmd.bin handled in txrx.cc, sched_cell.cc, sched_timepf.cc to control PHY and mac layers
  FILE* f2 = fopen("/mnt/tmp/agent_cmd.bin", "w");
  if (!f2){
    RIC_ERROR("Failed to open agent_cmd.bin");
    perror("fopen error");
    return false;
  }
  fwrite(pdu->msg, pdu->N_bytes, 1, f2);
  ...
  if (f) {
    ...
    size_t read = fread(iq_buffer, 614400, 1, f);
    if (read == 614400) {
